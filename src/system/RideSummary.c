...
bool RideSummary_FormatDurationShort(uint32_t seconds, char *out, size_t out_size) {
    ...
    out[0] = '\0';
    if (out_size < RIDE_SUMMARY_SHORT_TIME_MAX) {
        return false;
    }
    ...
    // Same shape either way: an unpadded leading field, a colon, a padded
    // remainder. Only the units change, and the absence of a second colon is
    // what says which.
    const int written = (hours > 0) ? snprintf(out, out_size, "%u:%02u", hours, minutes)
